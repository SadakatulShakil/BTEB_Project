...
class InitState extends State<QuizDetailsPage> {
  ...
  List<Answer> name = [];
  List<dynamic> values = [];
  Map<String, dynamic> answerMap = {"answers": "Here"};
  ...
  List<String> ansDemo = ['Banana', 'Mango', 'Apple', 'Chery'];
          ...
          child: FloatingActionButton.extended(
            ...
            onPressed: () {
              ...
              Navigator.push(context,
                  MaterialPageRoute(builder: (context) => ShowSelectRadio()));
            },
  ...
  Future getQuizQuestionData(
      String token, String quizId, String attemptId) async {
    ...
    if (quizQuestionData != null) {
      ...
      dumpQuestion =
          unescape.convert(quizQuestionData.questions![0].html.toString());
                          ...
                          children: [
                            Padding(
                              ...
                              child: Align(
                                alignment: Alignment.centerLeft,
                                child: parseQText(
                                    mQuestionSet), //Text(parse(HtmlUnescape().convert(mQuestionSet.html.toString())).getElementsByClassName("qtext")[0].text)
                              ),
                            ),
                          ]),
                    ),
  ...
  parseQText(Questions mQuestionSet) {
      ...
      if (value.localName == "p") {
        ...
        value.children.forEach((element) {
          if (element.localName == "img") {
            question +=
                "<img src='https://diginet-elearning.adnarchive.com/webservice/pluginfile.php" +
                    element.attributes["src"]
                        .toString()
                        .split("pluginfile.php")[1] +
                    "?token=$token' alt='Question Image' />";
          }
        });
      } else if (value.localName == "img") {
        question +=
            "<img src='https://diginet-elearning.adnarchive.com/webservice/pluginfile.php" +
                value.attributes["src"].toString().split("pluginfile.php")[1] +
                "?token=$token' alt='Question Image' />";
      }
    });
    ...
    quesHTML.getElementsByClassName("answer")[0].children.forEach((value) {
      ...
      if (value.localName != "input") {
        Map<dynamic, dynamic> answerOption =
            value.getElementsByTagName("input")[0].attributes;
    ...
    return Column(
      children: [
        Html(data: question),
        answers[0]['type'].toString() == 'hidden'
            ? RadioGroup(
